...
function checkUserMembership() {
    return fetch(SB_ajax_object.ajax_url + '?action=login_check_user_membership&nonce=' + SB_ajax_object.nonce)
...
quill1.clipboard.addMatcher('ham-dan.grammar-correction-added',
    mark('grammar-added'));
quill1.clipboard.addMatcher('ham-dan.grammar-correction-removed',
    mark('grammar-removed'));
quill1.clipboard.addMatcher('ham-dan.grammar-correction-punctuation',
    mark('grammar-punct'));
...
function hideUnderlines(flag) {
    ...
    const spans = textContainer.querySelectorAll('ham-dan.grammar-correction-added, ham-dan.grammar-correction-removed, ham-dan.grammar-correction-punctuation');
    ...
    if (noOfChanges != -1) {
        ...
        if (noOfChanges > 0 && !isExplanations) {
            ...
            if (htmlParts.length === 1) {
                ...
                grammerApiParallel('explanations', explanationParts).then((explanationResults) => {
                    // Combine results
...
document.querySelector('#genBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        clonedElement.querySelectorAll("ham-dan.grammar-correction-removed").forEach(hamDan => hamDan.remove());
        ...
        if (htmlParts.length === 1) {
            ...
            correctedResults = [await grammerApi('correction', {
        ...
        if (htmlParts.length === 1) {
            ...
            formatCallingParallelWithLoader(currentLanguage, formattingParts, diffHtml);
        }
    ...
    for (let i = 0; i < diff.length; i++) {
        ...
        if (current[0] === -1 && next && next[0] === 0) {
            ...
            if (prev && prev[0] === 0) {
                ...
                if (next[1] === " " && nextNext && nextNext[0] === -1 && isPunctuationString(nextNext[1]) &&
                    nextNextNext && nextNextNext[0] === 1 && nextNextNext[1].endsWith(nextNext[1])) {
        ...
        if (i > 0) {
            const prev = diff[i - 1];
            if (prev[0] !== 0 && prev[0] !== current[0]) { // Different operation type (add vs delete)
        ...
        if (!foundPunctuationOnlyMatch && next) {
            if (next[0] !== 0 && next[0] !== current[0]) { // Different operation type
...
function generateDiffHTML(diff) {
    ...
    for (let i = 0; i < diff.length; i++) {
        ...
        else if (part[0] === 0) { // Unchanged text
    ...
    function cleanElement(element) {
        ...
        if (element.tagName.toLowerCase() === 'span') {
            ...
            if (styleAttr) {
                ...
                if (styleAttr.includes('font-weight: 700') ||
...
function formatCallingWithLoader(language, userInputText, correctedText) {
    ...
    console.log("before tag removal ", convertStrongParagraphsToHeadings(removeHamDanTags(userInputText)));
    console.log("after tag removal ", convertPSpanstoBr(convertStrongParagraphsToHeadings(removeHamDanTags(userInputText))));
    // ✅ CLEAN HAM-DAN TAGS BEFORE SENDING
    let cleanedUserInput = convertPSpanstoBr(convertStrongParagraphsToHeadings(removeHamDanTags(userInputText)));
...
function formatCallingParallelWithLoader(language, formattingParts, fallbackDiffHtml) {
    ...
    const cleanedFormattingParts = formattingParts.map(part => ({
        ...
        userInputText: convertPSpanstoBr(convertStrongParagraphsToHeadings(removeHamDanTags(part.userInputText))),
...
function processGrammarExplanations(explanationResults) {
    ...
    const cleanParsedExplanations = parsedExplanations.filter(item => !item.reason.startsWith('Ingen ændring'));
...
function handleExplanationClick(event) {
    ...
    document.querySelectorAll('.explanation-item.active-explanation')
 ...
 */
function highlightWordInInput(word, threshold = 0.80) {
    /* ─── 1. Clean up ─────────────────────────────────────────────── */
    document.querySelectorAll('.ql-editor [style*="FFF1C2"]').forEach(el =>
    ...
    for (let winLen = Math.max(1, nLen - 1); winLen <= nLen + 2; winLen++) {
        for (let pos = 0; pos <= docLen - winLen; pos++) {
            ...
            for (let i = 0; i < winLen; i++) {
                if (charHasGrammar(pos + i)) { insideGrammar = true; break; }
            }
            ...
            // force visual yellow
            document.querySelectorAll('.ql-editor mark.word-highlight')
                .forEach(mark => {
                    mark.style.setProperty('background-color', '#FFF1C2', 'important');
                    mark.querySelectorAll('*').forEach(child =>
                        child.style.setProperty('background-color', '#FFF1C2', 'important')
                    );
                });
            return true;
        }
...
const resetSidebar = () => {
    ...
    const bubbleFun = document.querySelector('.correction-inner .demo-inner .hamdan-robot-container .hamdan-speech-bubble')
    ...
    fetch(SB_ajax_object.ajax_url, {
        ...
        .then(data => {
            if (data.success) {
                ...
                document.querySelector('.response-counter').textContent = `Tekst ${responseCount} ud af ${responseCount}`;
...
function analyseLoader(flag) {
    ...
    const bubble = document.querySelector('.correction-inner .demo-inner .hamdan-robot-container .hamdan-speech-bubble')
    if (flag) {
...
document.getElementById("submint_rewrite").addEventListener('click', inputRewritecall);
...
document.getElementById('convencing').addEventListener('click', () => handleRewrite('convencing'));
...
function sendRewriteRequest(buttonId) {
    ...
    let langForRewrite = Object.keys(languageMap).find(key => languageMap[key] === currentLanguage) || currentLanguage;
    // //// console.log("languague for rewrite", langForRewrite)
    formData.append('language', langForRewrite);
    switch (buttonId) {
        case 'convencing': formData.append('prompt_index', '0'); break;
        case 'simplify': formData.append('prompt_index', '1'); break;
        case 'elaborate': formData.append('prompt_index', '2'); break;
        case 'concise': formData.append('prompt_index', '3'); break;
    }
...
function adjustInputTextareaHeight(element = document.getElementById('inputText')) {
    ...
    const scrollTop = needsScrollHandling ? (window.pageYOffset || document.documentElement.scrollTop) : 0;
...
function robustHtmlDivider(htmlContent, maxLength = 500, targetSplits = 2) {
    ...
    const atomic = new Set([
        ...
        'BLOCKQUOTE', 'SECTION', 'ARTICLE', 'HEADER', 'FOOTER', 'NAV', 'ASIDE', 'MAIN'
    ]);
...
function processComplexHtml(htmlContent, maxLength = 500) {
    ...
    const cleaned = (typeof convertBulletListToUl === 'function' &&
                    ...
                    if (item.types.includes('text/html')) {
                        ...
                        if (cleanedHTML.trim()) {
                            if (isEmpty) {
                                ...
                                const pasteIndex = selection ? selection.index : quill1.getLength();

                                // Store the length before pasting to calculate the pasted content length
                                const lengthBeforePaste = quill1.getLength();

                                quill1.clipboard.dangerouslyPasteHTML(pasteIndex, cleanedHTML, 'user');
        ...
        if (text) {
            ...
            if (text.trim()) {
                if (isEmpty) {
                    ...
                    if (selectionLength > 0) quill1.deleteText(insertIndex, selectionLength);
        ...
        (e) => {
            ...
            if (cb && cb.items) {
                for (const it of cb.items) {
                    if (it.kind === 'string' && /text\/(plain|html)/.test(it.type)) { hasText = true; break; }
                }
...
const handleQuillCopy = async () => {
    try {
        ...
        const { html: htmlContent, text: textContent } = processQuillContentForCopy(quill1);
...
imageUploadInput.setAttribute('accept', 'image/*,.pdf,.docx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document');
...
document.getElementById('imageUpload').addEventListener('change', async function (event) {
    ...
    try {
        ...
        const docxFiles = files.filter(file => file.type.toLowerCase() === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
        ...
        if (imageFiles.length > 0) {
            ...
            for (let i = 0; i < imageFiles.length; i++) {
                ...
                let imageText = '';
                if (hasExifData || imageFiles[i].type.toLowerCase().includes('heic') ||
            ...
            try {
                ...
                tempImg.onload = function () {
                    EXIF.getData(tempImg, function () {
                        ...
                        const hasCameraData = allTags.Make ||
                            allTags.Model ||
...
async function processImageWithOCRForPDF(file) {
    ...
    if (typeof file === 'string') {
        try {
            const { data: { text } } = await Tesseract.recognize(file, 'dan');
            return text;
        } catch (error) {
        ...
        return new Promise((resolve) => {
            reader.onload = async function () {
                try {
                    const { data: { text } } = await Tesseract.recognize(reader.result, 'dan');
                    resolve(text);
                } catch (error) {
                    console.error('OCR Error:', error);
                    resolve('');
                ...
                try {
                    ...
                    if (imageData) {
                        ...
                        const enhancedConfig = {
                            ...
                            tessedit_char_whitelist: 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZæøåÆØÅ0123456789.,;:!?(){}[]<>@#$%^&*+=-_\'"`~§°€/\\|'
                        };
...
async function toggleRecording(buttonId) {
    try {
        ...
        if (!isListening[buttonId]) {
            ...
            if (buttonTextMap[buttonId] === "inputText" && typeof quill1 !== 'undefined') {
                quill1.focus();
                if (quillCursorPosition !== null) {
                    quill1.setSelection(quillCursorPosition, 0);
                }
...
async function connectToSpeechServer(buttonId) {
    try {
        ...
        const socket = new WebSocket(`${SERVER_URL}?api_key=${API_KEY}&language=${language}&model=nova-2&interim_results=false `);
                            ...
                            if (inputText) {
                                if (inputText.id === "inputText") {
                                    ...
                                    if (quillCursorPosition !== null) {
                                        ...
                                        const selection = quill1.getSelection();
                                        insertPosition = selection ? selection.index : quill1.getLength() - 1;
                                    }
...
function saveResponse(response) {
    ...
    fetch(SB_ajax_object.ajax_url, {
        ...
        body: 'action=korrektur_save_response&response=' + encodeURIComponent(response) + '&nonce=' + SB_ajax_object.nonce
    })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function getSavedResponses() {
    ...
    return fetch(SB_ajax_object.ajax_url + '?action=korrektur_get_user_responses&nonce=' + SB_ajax_object.nonce)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function deleteResponse(responseId) {
    ...
    fetch(SB_ajax_object.ajax_url, {
        ...
        body: 'action=korrektur_delete_response&response_id=' + responseId + '&nonce=' + SB_ajax_object.nonce
    })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function displaySavedResponses() {
    ...
    historyLoader(true);
    getSavedResponses().then(savedResponses => {
...
function attachCopyAndDeleteEventListeners(savedResponses) {
    ...
    document.querySelectorAll('.copy-btn1').forEach(button => {
        button.addEventListener('click', function () {
            const responseContainer = this.closest('.saved-response').querySelector('.response-text-area');
    ...
    document.addEventListener('copy', (e) => {
        ...
        const isInResponseArea = container.closest && container.closest('.response-text-area') ||
            (container.parentNode && container.parentNode.closest &&
    ...
    if (clearHistoryButton) {
        clearHistoryButton.addEventListener('click', function () {
            ...
            fetch(SB_ajax_object.ajax_url, {
                ...
                body: 'action=korrektur_delete_all_user_responses&nonce=' + SB_ajax_object.nonce
            })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
...
document.addEventListener('DOMContentLoaded', function () {
    document.getElementById('showSavedResponsesBtn').addEventListener('click', openPopup);
...
document.querySelectorAll('.download-option').forEach(option => {
    option.addEventListener('click', async () => {
        ...
        const formattedContent = removeInlineStyles(
            removeHamDanTags(removeMarkTags(
                sanitizeHtmlContentForDownload(quill1.root.innerHTML)
            ))
        );
...
class QuillSelectionToolbar {
        ...
        document.addEventListener('selectionchange', () => {
            ...
            if (selection.rangeCount > 0 && !selection.isCollapsed) {
                ...
                if (quillEditor && quillEditor.contains(range.commonAncestorContainer)) {
                    ...
                    if (text && !this.isSelecting) { // Only if we're not in the middle of a mouse selection
    ...
    isSelectionWithinQuill(range, quillEditor) {
        try {
            ...
            const startInQuill = quillEditor.contains(startContainer) ||
                (startContainer.nodeType === Node.TEXT_NODE && quillEditor.contains(startContainer.parentNode));
            const endInQuill = quillEditor.contains(endContainer) ||
    ...
    fallbackCopy(htmlContent = '', textContent = '') {
        try {
            ...
            if (navigator.clipboard && navigator.clipboard.writeText) {
                navigator.clipboard.writeText(textContent || this.selectedText)
